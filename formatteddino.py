...
class Dino():
    def __init__(self):
        ...
        self.img.append(
            ImageTk.PhotoImage(
                Image.open(os.path.join(FILEPATH, 'assets/left.gif'))))
        self.img.append(
            ImageTk.PhotoImage(
                Image.open(os.path.join(FILEPATH, 'assets/right.gif'))))
        self.jump = ImageTk.PhotoImage(
            Image.open(os.path.join(FILEPATH, 'assets/jump.gif')))
        self.dead = ImageTk.PhotoImage(
            Image.open(os.path.join(FILEPATH, 'assets/stop.gif')))
        ...
        self.h_indicator = w.create_text(
            100, 26, text='', font='Minecraftia 12', fill='#535353')
        self.dino_h = 0
        self.h_circle = w.create_oval(
            100 - self.dino_h,
            18 - self.dino_h,
            100 + self.dino_h,
            18 + self.dino_h,
    ...
    def height_loop(self):
        ...
        if self.dino_h >= 0:
            ...
            #if c>=20 : c=20
            self.h_circle = w.create_oval(
                100 - c, 14 - c, 100 + c, 14 + c, outline='#535353')
        else:
            c = 0
            w.delete(self.h_circle)
...
class Clouds():
    ...
    def add(self):
        ...
        num = w.create_image(
            width, random.randint(20, height - 230), image=w.cloud)
        w.tag_lower(num)
...
def detect_collision():
    ...
    for obstacle in obstacles:
        ...
        d = ((x_min + 5 > x_d + 7), (x_max - 5 < x_d - 7),
             (y_min + 5 > y_d + 17), (y_max - 5 < y_d - 17))
        if not any(d):
...
def restart(e):
    ...
    if if_dead:
        ...
        with open(d, 'r') as f:
            hiscore = f.read()
        if auto:
            auto_text = w.create_text(
                50, 20, text='AUTO', font='Minecraftia 20', fill='#535353')
        else:
            auto_text = w.create_text(
                50, 20, text='', font='Minecraftia 20', fill='#535353')
        old_state = auto
        hiscore_text = w.create_text(
            width - 160,
            20,
            text='HI ' + str(hiscore).zfill(5),
            font='Minecraftia 20',
            fill='#535353')
        init_floor()
        timer = time.time()
...
hiscore_text = w.create_text(
    ...
    font='Minecraftia 20',
    fill='#535353')
w.gameover = ImageTk.PhotoImage(
    Image.open(os.path.join(FILEPATH, 'assets/gameover.gif')))
...
obs = []
paths = (os.path.join(FILEPATH, 'assets/ca1.gif'),
